...
public interface TaskCompletedRepository extends JpaRepository<TaskCompleted, Integer> {
    ...
    @Query(value = "SELECT JSON_ARRAYAGG(JSON_OBJECT('id_task', task_completed.id_task_completed, 'name', task.name, 'username', user.username, 'photo', user.photo)) FROM task_completed JOIN task ON task.id_task = task_completed.id_task join user on user.id_user = task_completed.id_user WHERE task_completed.status = 0 and task.id_team = ?1", nativeQuery = true)
