...
public interface TaskCompletedRepository extends JpaRepository<TaskCompleted, Integer> {
    ...
    @Query(value = "UPDATE task_completed SET task_completed.revision_date= ?1, task_completed.status= ?2, task_completed.comment= ?3 WHERE task_completed.id_task = ?4 ", nativeQuery = true)
    String update(Timestamp revision_date, int status, String comment, int idTaskCompleted);

    @Query(value = "SELECT JSON_ARRAYAGG(JSON_OBJECT('name', task.name, 'username', user.name, 'photo', user.photo)) FROM task_completed JOIN task ON task.id_task = task_completed.id_task join user on user.id_user = task_completed.id_user WHERE task_completed.status = 0 and task.id_team = ?1", nativeQuery = true)
